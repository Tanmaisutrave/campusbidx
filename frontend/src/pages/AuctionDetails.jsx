...
const AuctionDetails = () => {
  ...
  // Check if this is a demo auction by matching the prefixed ID
  const demoAuction = DEMO_AUCTIONS.find((d) => d._id === id);

  const fetchAuction = async () => {
    // Demo auctions are never fetched from the backend — serve locally
    if (demoAuction) {
      setAuction(demoAuction);
      setLoading(false);
      return;
    }
    try {
  ...
  const isDemo = Boolean(auction.isDemo);
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {isDemo && (
        <div className="mb-6 flex items-start gap-3 bg-orange-50 border border-orange-200 rounded-xl px-5 py-4">
          <span className="text-xl">🏷️</span>
          <div>
            <p className="font-semibold text-orange-700 text-sm">Demo Auction Item</p>
            <p className="text-orange-600 text-xs mt-0.5">
              This is a sample listing to showcase the CampusBidX platform. It is not a real auction and bidding is disabled.
              Real auctions will appear on the homepage once posted by campus students.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div>
          ...
          <div className="bg-white rounded-2xl shadow-md p-6">
            <div className="flex items-start justify-between mb-3">
              ...
              {isDemo ? (
                <span className="text-xs font-semibold px-3 py-1 rounded-full bg-orange-100 text-orange-600 border border-orange-200 shrink-0 ml-2">
                  DEMO
                </span>
              ) : (
            ...
            <div className="border-t pt-4 text-sm text-gray-500">
              ...
              {auction.seller?.studentId && auction.seller.studentId !== 'DEMO' && (
                <p>Student ID: <span className="font-medium text-gray-700">{auction.seller.studentId}</span></p>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-2xl shadow-md p-6">
            ...
            {/* Countdown — only for real auctions with a valid endTime */}
            {!isDemo && (
            ...
            {/* Bid area — disabled for demo auctions, real BidForm for real auctions */}
            {isDemo ? (
              <div className="bg-orange-50 border border-orange-200 rounded-xl p-4 text-center">
                <p className="text-orange-600 text-sm font-medium">🔒 Bidding Disabled</p>
                <p className="text-orange-500 text-xs mt-1">
                  This is a demo auction item. Bidding is not available for sample listings.
                </p>
              </div>
            ) : (
          ...
          <div className="bg-white rounded-2xl shadow-md p-6">
            ...
            {auction.bids?.length > 0 ? (
              ...
              <p className="text-gray-400 text-sm text-center py-4">
                {isDemo ? 'No bids — this is a demo item.' : 'No bids yet. Be the first!'}
              </p>
            )}
          </div>
        </div>
