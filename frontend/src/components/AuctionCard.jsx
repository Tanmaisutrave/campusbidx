...
const AuctionCard = ({ auction }) => {
  ...
  const { _id, title, startingPrice, currentBid, endTime, image, category, status, isDemo } = auction;
  ...
  return (
    ...
    >
      <div className="relative overflow-hidden h-48">
        ...
        {/* Demo badge — only shown on static demo auction cards */}
        {isDemo && (
          <span className="absolute bottom-2 right-2 bg-orange-100 text-orange-600 border border-orange-300 text-xs font-semibold px-2 py-0.5 rounded-full">
            DEMO
          </span>
        )}
      </div>
      <div className="p-4">
        ...
        <div className="border-t pt-3">
          {/* Demo auctions have no endTime — show a static label instead of a live countdown */}
          {isDemo ? (
            <>
              <p className="text-xs text-gray-500 mb-1">Availability</p>
              <span className="text-orange-500 font-semibold text-sm">Sample Auction</span>
            </>
          ) : (
            <>
